#!/bin/bash
# ansible-local container launcher
...
IMAGE_NAME="homelab-ansible-local"
ECR_IMAGE="${ECR_REGISTRY}/homelab/ansible-local:latest"

# Path to the Ansible repo (playbooks/ + inventory/) mounted read-only at /workspace
REPO_PATH="${ANSIBLE_REPO_PATH:-$HOME/src/ansible}"
...
# Verify the Ansible repo exists
if [ ! -d "$REPO_PATH" ]; then
    error "Ansible repository not found at $REPO_PATH (set ANSIBLE_REPO_PATH)"
    exit 1
fi
...
info "Starting ansible-local container..."

# Build docker run command
DOCKER_CMD="docker run $DOCKER_FLAGS --rm --name ansible-local-session"
