...
KUBECONFIG_CONTENT=$(aws secretsmanager get-secret-value \
    --secret-id homelab/kubeconfig/k3s-main \
    --query SecretString --output text 2>/dev/null)

if [ -z "$KUBECONFIG_CONTENT" ]; then
    echo "ERROR: Failed to fetch kubeconfig from homelab/kubeconfig/k3s-main"
    exit 1
fi
...
echo "Kubeconfig loaded for k3s-main cluster"

# Test cluster connectivity
echo ""
echo "Testing cluster connectivity..."
if kubectl cluster-info >/dev/null 2>&1; then
    echo "Successfully connected to k3s-main cluster"
    echo ""
    kubectl get nodes
else
    echo "WARNING: Cannot connect to k3s-main cluster"
    echo "Check network connectivity to 192.0.2.68:6443"
fi
