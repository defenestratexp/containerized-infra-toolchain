#!/bin/bash
# kubectl-k3s-public container launcher
...
IMAGE_NAME="homelab-kubectl-k3s-public"
ECR_IMAGE="${ECR_REGISTRY}/homelab/kubectl-k3s-public:latest"
...
if ! docker image inspect "$IMAGE_NAME" >/dev/null 2>&1; then
    # Try local build first (no ECR image yet)
...
info "Starting kubectl-k3s-public container..."

# Build docker run command
DOCKER_CMD="docker run $DOCKER_FLAGS --rm --name kubectl-k3s-public-session"
