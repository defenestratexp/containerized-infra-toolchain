#!/bin/bash
# Proxmox API container launcher
...
IMAGE_NAME="homelab-proxmox"
ECR_IMAGE="${ECR_REGISTRY}/homelab/proxmox:latest"
...
# Default to pve1 host, can override with PROXMOX_HOST env var
PROXMOX_HOST="${PROXMOX_HOST:-pve1}"

info "Starting proxmox container (target: $PROXMOX_HOST)..."

# Build docker run command
DOCKER_CMD="docker run $DOCKER_FLAGS --rm --name proxmox-session"
...
DOCKER_CMD="$DOCKER_CMD -e PROXMOX_HOST=$PROXMOX_HOST"
...
# Add pxm prefix and any command arguments
if [ $# -gt 0 ]; then
    DOCKER_CMD="$DOCKER_CMD pxm $*"
fi
